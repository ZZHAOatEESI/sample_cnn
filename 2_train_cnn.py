...
def cnn_model(input_shape):
    ...
    X = Conv2D(32, (1,6), strides = (1, 1), name = 'conv0', data_format = 'channels_last')(X_input)
    X = BatchNormalization(axis = -1, name = 'bn0')(X)
    X = Activation('relu')(X)
    
    X = MaxPooling2D((1,4), name='max_pool_0', data_format = 'channels_last')(X)
    
    X = Conv2D(64, (1,11), strides = (1, 1), name = 'conv1', data_format = 'channels_last')(X)
    X = BatchNormalization(axis = -1, name = 'bn1')(X)
    X = Activation('relu')(X)
    
    X = MaxPooling2D((1,2), name='max_pool_1', data_format = 'channels_last')(X)
    
    X = Conv2D(128, (1,10), strides = (1, 1), name = 'conv2', data_format = 'channels_last')(X)
    X = BatchNormalization(axis = -1, name = 'bn2')(X)
    X = Activation('relu')(X)
    
    X = Conv2D(1, (1,1), strides = (1, 1), name = 'conv3', data_format = 'channels_last')(X)
    X = Activation('relu')(X)
...
model.fit_generator(generator=training_generator,epochs=15)
model.save('sample_cnn_model_2.pkl')
